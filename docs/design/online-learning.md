...
| L4 | `.../manager.py` | `LearningManager`：实现 `LearningHooks`（`wrap_handle`/`on_finished`，被 `PlayManager` 调用）；apply 管线（建表 → 门禁 → 发布）、`start_auto` 后台 daemon、状态 API |
| L3 | `layer3_solvers/hybrid/opponent_model.py` | `EmpiricalModel`（Laplace 平滑 + 未见信息集回退 uniform，仿 `CFRTableModel`）；`HybridSolver.learn_online()` 增量合并（duck-typed 信号，L3 不 import L4） |
...
CLI：`python -m layer4_interface.frontend.platform.server --learning-interval N` 后台自动；
手动 apply 走 `/api/learning/apply` 或平台前端页面（原 `demos.online_learning_eval` 入口已随
demos 清理移除，功能由平台服务与前端页面覆盖）。
