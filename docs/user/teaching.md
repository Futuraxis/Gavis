...
- 支持全部平台游戏：月亮棋 / 随机五子棋 / 德州扑克 / 麻将七变种（含国际）/ UNO 六变体 / 自定义游戏。
...
5. **对局记录**：历史记录 `meta.teaching` 标记教学局（旧记录缺省 `null` = 非教学局）；平台「对局记录」页可在展开详情里看到「教学对局：是/否」（`/api/history` 的 `matches[].teaching`）。
