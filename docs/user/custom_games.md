...
玩家可以在平台里用**自然语言规则**或**已有游戏的变体**直接生成可对弈的新游戏——
这是 Layer 1（规则翻译）被接入对弈工作流后的主入口。翻译 → 校验 → 规则族识别 →
注册 → 大厅出现 → 正常人机对弈，全链路自动完成。
...
2. **模式二：基于模板变体（variant）**——选一个基础游戏（月亮棋/五子棋/德州/
   麻将/狼人杀），再描述要做的改动，例如「棋盘改成 7×7，五子连珠获胜，
   每步落子后 30% 概率抹去一格」。
...
| `poker` 扑克 | hole/community 数组 + raise/call/fold 动作 | 扑克桌 | Hybrid（不完全信息搜索） |
...
- 所有自定义/变体规则都经过 **schema 校验 + L2 引擎冒烟校验**才会注册；
  引擎一律以 `allow_codegen=False`（纯解释器路径）构造——
...
- `layer1_translator/variant_translator.py` — 变体翻译（确定性参数路径 + LLM 全模板改写修复循环，
  输出必过 `EngineValidator`）
- `layer4_interface/frontend/platform/families/` — 规则族包（自动发现；`detect` 判定 + `build_spec`
  产出 `GameSpec`；`grid/poker/mahjong/social` 四族）
- `layer4_interface/frontend/platform/custom_games.py` — `CustomGameStore`（持久化）+ `CustomGameRegistry`
  （翻译→校验→族识别→建 spec→注册 编排）
- 平台会话/求解器装配：`session.py` 查表回退到自定义注册表；
  `train-cli/games.py` 的 `create_solver(..., allow_unknown=True)` 为未登记自定义游戏
  装配通用运行时求解器（`mcts/random/ollama/mahjong/hybrid` 白名单，其余名字仍拒绝）