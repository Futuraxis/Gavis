...
- **人数**：4..12 人（默认 8），运行时用 `player_count` 覆盖
  ...
  `EngineSpec.player_count`）。4..12 边界由生成器 CLI help 声明
  （JSON 内无显式 min/max 字段；人数由 variants 的 `player_ids` 公式
  生成 p0..p{N-1}）。
- **配比**：固定 1 卧底 + 1 白板 + N 平民（`role_pool` 公式：
  concat(["undercover","blank"], N-2 个 "civilian")）。
- **主题（词对类别）+ 难度（词对相似度）**：`variant` 形如 `{theme}_{difficulty}`
  —— 6 主题 × 3 难度 = 18 个 variant，全部在 `variants.options` 声明
  （dict，键=变体名），未知 variant → ValueError。
  ...
  - **难度**（`easy`/`normal`/`hard`）：词对相似度档——`easy` 词对差异大（如
    ...
    猎豹/花豹、骡子/马）。每档每主题约 3 对，每局从该档词池**随机抽一对**。

## 平台对弈

平台大厅已内置本游戏（`layer4_interface/frontend/platform/games.py`
注册表，social 族）：

- **入口**：启动平台后浏览器打开 `http://127.0.0.1:8770`，大厅点「谁是卧底」
  →「开始对局」；对局界面为社交聊天桌（身份/词语/存活/发言记录/投票），
  与自定义社交游戏共用。
...
- **隐藏信息红线**：快照只从 `engine.project_observation` 构建——你只能看到
  自己的身份与词语；他人身份/词语（含 AI 的）在终局前不出现。

## 难度与节奏

平台统一 **`difficulty × pacing` 3×3 网格**（与麻将选 variant、狼人选难度
共用同一套平台契约）。

| 维度 | 取值 | 影响（谁是卧底） |
|---|---|---|
| `difficulty` | `easy`/`normal`/`hard` | **词对相似度档**——`easy` 词对差异大、线索好抓；`hard` 极易混淆（如菠萝/凤梨、猎豹/花豹、骡子/马）。**同时**叠加 AI 发言强度提示（`_UNDERCOVER_HINT`：`easy` 直白描述、`normal` 克制模糊、`hard` 高度模仿/误导）。 |
| `pacing` | `fast`/`standard`/`slow` | **AI 发言温度**——`fast` 高温(0.9)发散、描述跳脱易露馅；`standard`(0.7)平衡；`slow`(0.5)精准、强伪装、线索少。 |

- **自适应模式（`adaptive=true`）**：词对档锚定 `normal`（AI 强度仍按自适应
  档调节），只有显式 `difficulty` 才选 `easy`/`hard` 词对池。
- **主题**与难度正交：选 `animal_hard` 即「动物·极易混淆」词池；选
  `food_normal` 即「食物·同类相近」。每档每主题约 3 对，每局随机抽一对，
  重复游玩不重复。
...
1. **描述**：所有存活玩家按座位轮流用一句话描述自己的词（`speak`，自由文本，
   不能说出词本身；`text` 参数走 v5.2 text 参数预制能力，不参与合法动作枚举）。
...
- **身份隐藏**：不开 `my_role` 视图——每个玩家只看**自己的词**
  （`my_word` 按 viewer 过滤）；平民/卧底不知道自己的身份标签。
  白板看到「白板」(无词) 自知是白板。
- 死后身份与词语公开（`dead_roles` / `dead_words` 保留 `alive==0` 的行）。
- 描述、投票、自爆事件、出局记录全员可见；`env` 字段（轮次、出局者、胜方）
  全员可见。
...
- 规则源文件：`scripts/_gen_undercover.py`（argparse：`--players` 默认 8、
  `--max-players` 默认 12、`--theme` 默认 fruit、`--difficulty`、
  `--out` 默认 rules/undercover.json）；改规则后运行
