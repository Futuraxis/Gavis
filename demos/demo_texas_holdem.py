...
def play_one_hand(engine, solver, verbose=True):
    ...
    if verbose:
        ...
        print("═" * 56)
        print(
            f"你的底牌(SB): {render_cards(state['_arrays']['sb_hole'])}"
    ...
    while not engine.is_terminal(state):
        ...
        if nt == "chance":
            ...
            if verbose and after > before:
                print(f"\n  {STREETS[state['env']['street']]}公共牌: {render_cards(state['_arrays']['community'])}")
            continue
        if nt != "player":
            break
    ...
    if verbose:
        ...
        print(f"SB: {render_cards(state['_arrays']['sb_hole'])}   BB: {render_cards(state['_arrays']['bb_hole'])}")
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="train a CFR prior for the hybrid (takes seconds to minutes); 0 = no prior, uniform opponent model",
    )
