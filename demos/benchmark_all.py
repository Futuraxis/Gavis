...
def main():
    ...
    parser.add_argument(
        "--game", type=str, default="moon_chess", choices=["moon_chess", "stochastic_gomoku", "texas_holdem"]
    )
    parser.add_argument("--episodes", type=int, default=10, help="Training episodes (only affects PPO/PSRO)")
    parser.add_argument(
        "--solvers", type=str, nargs="+", default=["mcts", "cfr", "ppo", "psro"], choices=["mcts", "cfr", "ppo", "psro"]
    )
    ...
    for r in results:
        print(
            f"  {r['solver']:12s} {r['moves']:6d} {r['elapsed_s']:10.3f} {r['avg_s_per_move']:8.4f} {str(r['winner'] or '平局'):8s}"
        )
