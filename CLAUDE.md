...
```
rules/                 游戏规则 JSON (v5.2, 零 BUILTIN + variants 声明式: texas_holdem / mahjong / werewolf)
...
- `rules/mahjong.json` 由 `_gen_mahjong.py` 生成（三变种 × 2/4 人由
  variants 声明；胡牌判定的 `best5` 等手感常数在 `constants`），改规则
  改生成器再重新生成
