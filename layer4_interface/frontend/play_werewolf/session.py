        ...
        if self.my_turn:
            for action in self.engine.get_legal_actions(self.state):
                ...
                if action.template_id == 'speak':
                    legal.append({'id': action.template_id,
                                  'intent': p.get('intent', {}).get('id')})
                else:
                    legal.append({'id': action.template_id,
                                  'target': p.get('target', {}).get('id')})
...
class PlayManager:
    ...
    def start(self, players: int = 9, wolves: int = 3, model: str = 'qwen3:8b',
              human_pid: str | None = None) -> GameSession:
        ...
        ai_solvers = {
            pid: OllamaSolver(engine, OllamaConfig(model=model), player_id=pid)
            for pid in pids if pid != human_pid
        }
        session = GameSession(
