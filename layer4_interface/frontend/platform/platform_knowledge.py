...
PLATFORM_TOPICS: tuple[PlatformTopic, ...] = (
    ...
    PlatformTopic(
        key="create",
        title="创建自定义游戏",
        summary="直接把规则说给助手听即可创建；也可进顶部「创建游戏」页用表单。",
        text=(
            "两种入口：\n"
            "· **对话里**：直接描述规则（“做一个 8×8 四子连珠的游戏，叫四子棋”），或说"
            "“基于随机五子棋改成 15×15 连五”——助手把它交给规则翻译器创建，成功后可立即"
            "说“玩四子棋”开局；规则说得太笼统时助手会追问，不会替你编规则。\n"
            "· **顶部导航「创建游戏」（/create）**：表单里写规则或选模板变体，另有 LLM 翻译"
            "开关与自定义游戏管理（删除）。\n"
            "两种模式：\n"
            ...
            "大厅自定义卡片可删除；对话里默认走确定性模板翻译（快），需要 LLM 翻译时可在"
            "创建页勾选（模型不可用自动回落确定性翻译）。"
        ),
    ...
    PlatformTopic(
        key="settings",
        title="设置",
        summary="说“打开设置”/“怎么改难度”/“换个风格”；可调助手性格、AI 难度、声音、主题等。",
        text=(
            "助手性格可以直接在对话里换：说“换成温柔陪伴/认真教学/轻松吐槽/高冷竞技”"
            "（或“换个风格”让平台给选项）立刻生效并写进你的档案。\n"
