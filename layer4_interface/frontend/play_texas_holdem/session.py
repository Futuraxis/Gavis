...
from layer2_engine.core.poker_utils import poker_hand_name, poker_payoff, PLAYER_SB, PLAYER_BB
...
class PlayManager:
    ...
    def start(self, playerColor: str, difficulty: str) -> GameSession:
        if playerColor == 'random':
            playerColor = PLAYER_SB if uuid.uuid4().int % 2 == 0 else PLAYER_BB
        if playerColor not in (PLAYER_SB, PLAYER_BB):
            raise PlayError(f'unknown seat: {playerColor}')
        ...
        session = GameSession(
            game_id=game_id,
            player_pid=playerColor,  # type: ignore[arg-type]
