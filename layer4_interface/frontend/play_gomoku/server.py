"""Stochastic Gomoku play app — human vs AI, independent HTTP server.

API (same shape as play_moon_chess):
...
Usage:  python -m layer4_interface.frontend.play_gomoku.server [--host H] [--port P]
"""
...
class GomokuHandler(SimpleHTTPRequestHandler):
    server_version = "GavisGomoku/0.1"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Start the Stochastic Gomoku play app.")
    ...
    GomokuHandler.manager = PlayManager(provider=default_provider)
    server = ThreadingHTTPServer((args.host, args.port), GomokuHandler)
    print(f"Stochastic Gomoku app running at http://{args.host}:{args.port}/")
    try:
