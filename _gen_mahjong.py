...
def FLAT2(lst_of_lists):
    ...
    """
    return CONCAT(AT(lst_of_lists, C(0)), AT(lst_of_lists, C(1)), AT(lst_of_lists, C(2)), AT(lst_of_lists, C(3)))
...
def _cover_ok(selected_expr, hand_expr):
    ...
    return GTE(ADD(_min_sel_have(selected_expr, hand_expr), _hand_count(hand_expr, V("$constants.wild_tile"))), C(14))
...
def _meld_pool(hand_expr):
    ...
    pung = MAP(FILTER(GROUP(hand_expr), GTE(ADD(GET(V("$node"), "count"), wild), C(3))), GET(V("$node"), "key"))
...
def _actions():
    return [
        ...
        },
        {
            ...
            "legal": CALL("is_win_hand", CALL("hand_of", V("$env.turn"))),
        ...
        },
        {
            ...
            "legal": CALL("is_win_hand", CONCAT(CALL("hand_of", CLAIM_ACTOR), SINGLE(V("tile")))),
        ...
        },
        {
            ...
            "legal": AND(EQ(V("$env.claim_index"), C(0)), GTE(COUNT(CALL("hand_of", CLAIM_ACTOR)), C(2))),
...
def _effectors():
    ...
    return {
        ...
        "do_claim_chi": {
            ...
            "ops": [
                _set_env("actor", actor),
                {"op": "forEach", "list": V("tiles"), "do": [_remove(actor_hand_tmpl, V("$item"))]},
        ...
        "do_win": {
            ...
            "ops": [
                ...
                _branch(
                    ...
                    [
                        _branch(
                            OR(GTE(COUNT(V("$env.done")), C(2)), EQ(V("$env.wall_count"), C(0))),
...
def _aliases():
    ...
    fans = {
        ...
        "fan_pengpenghu": {
            "description": "碰碰胡 (approx: four triplets)",
            "params": ["hand"],
            "expr": EQ(COUNT(FILTER(GROUP(hand), GTE(GET(V("$node"), "count"), C(3)))), C(4)),
        },
        "fan_qingyise": {
            "description": "清一色",
            "params": ["hand"],
    ...
    for name in fan_order:
        fan_sum_expr = ADD(fan_sum_expr, MUL(CALL(name, hand), C(fan_value[name])))
    ...
    return {
        ...
        "is_win_hand": {
            "description": "Winning hand: 7 pairs, thirteen orphans, or standard form with coverage",
            "params": ["hand"],
            "expr": OR(
                CALL("is_qidui", hand),
                AND(EQ(COUNT(hand), C(14)), ALL(V("$constants.thirteen_orphans"), {"contains": [hand, V("$node")]})),
                _standard_win(hand),
            ),
        },
        "fan_sum": {
