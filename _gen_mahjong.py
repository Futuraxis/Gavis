...
def FLAT2(lst_of_lists):
    ...
    """
    return CONCAT(
        AT(lst_of_lists, C(0)), AT(lst_of_lists, C(1)), AT(lst_of_lists, C(2)), AT(lst_of_lists, C(3)),
        AT(lst_of_lists, C(4)),
    )
...
def _meld_pool(hand_expr):
    ...
    pung = MAP(
        ...
        {"map": {"list": {"range": {"from": {"const": 0}, "to": {"const": 3}}}, "as": "$i", "expr": GET(V("$node"), "key")}},
    )
...
def _actions():
    return [
        ...
        },
        {
            ...
            # Sichuan 血战到底 bans chi entirely; other variants keep it.
...
def _aliases():
    ...
    return {
        ...
        "is_win_hand": {
            "description": (
                "Winning hand per variant: guangdong/hongzhong/blood = 7 pairs, "
            ...
            "expr": IF(
                EQ(V("$constants.variant"), C("taiwan")),
                OR(CALL("is_licu", hand), _standard_win(hand)),
                IF(
                    EQ(V("$constants.variant"), C("changsha")),
                    OR(
                        CALL("is_qidui", hand),
                        CALL("fan_jiangjianghu", hand),
                        # 小胡: standard 4 melds + pair, pair must be 2/5/8.
                        _standard_win(hand, _pair_pool_258),
                        # 大胡 (碰碰胡/清一色): 乱将 — any pair + standard structure.
                        AND(_standard_win(hand), OR(CALL("fan_pengpenghu", hand), CALL("fan_qingyise", hand))),
                    ),
                    IF(
                        EQ(V("$constants.variant"), C("sichuan")),
                        AND(
                            ...
                            # 缺一门 gate: fewer than 3 suits (no honors in deck).
                            NOT(EQ(COUNT(DISTINCT(suits_noz)), C(3))),
                        ),
                        OR(
                            CALL("is_qidui", hand),
                            AND(
    ...
    return {
        ...
        "variants": {
            ...
            "options": {
                ...
                "changsha": {"constants": {"tile_ids": TILE_IDS_108, "pair_258": PAIR_258, "fan_pay": FAN_PAY_CHANGSHA, "gang_actions": GANG_ACTIONS}},
