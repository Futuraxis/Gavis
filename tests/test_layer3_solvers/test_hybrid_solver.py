...
class TestHybridCfrTable:
    def test_train_builds_full_table(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            solver = HybridSolver(engine, HybridConfig(
                seed=42, cfr_iterations=30, cfr_depth_limit=4, cfr_table_path=path))
    ...
    def test_table_mode_reload(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            path = os.path.join(tmp, 'cfr.json')
            trainer = HybridSolver(engine, HybridConfig(
                seed=42, cfr_iterations=30, cfr_depth_limit=4, cfr_table_path=path))
...
class TestHybridPoker:
    ...
    def _solver(texas: TexasHoldemAdapter, budget: int = 100) -> HybridSolver:
        return HybridSolver(texas, HybridConfig(
            seed=42, imperfect_information=True, mcts_budget=budget))
    ...
    def test_poker_chance_expansion(self, texas: TexasHoldemAdapter):
        ...
        assert all(abs(c.total_value) <= c.visits * 100 + 1e-9
    ...
    def test_poker_cfr_prior(self, texas: TexasHoldemAdapter):
        ...
        solver = HybridSolver(texas, HybridConfig(
            seed=42, cfr_iterations=15, cfr_depth_limit=10))
        solver.train(1, verbose=False)
