...
class TestMoonChessBasics:
    def test_create_initial_state(self, adapter: MoonChessAdapter):
        ...
        assert len(state["_board"]) == 9
        assert all(c is None for c in state["_board"])
        assert "pieceOrder" in state["env"]
        assert state["env"]["pieceOrder"]["p_black"] == []
        assert state["env"]["pieceOrder"]["p_white"] == []
...
class TestMoonChessGameplay:
    def test_place_piece(self, adapter: MoonChessAdapter):
        ...
        assert new_state["_board"][0] == "p_black"
        assert len(new_state["env"]["pieceOrder"]["p_black"]) == 1
        assert new_state["env"]["pieceOrder"]["p_black"][0] == "cell_0_0"
    ...
    def test_fifo_eviction(self, adapter: MoonChessAdapter):
        ...
        # Black and white alternate. We'll place 4 times for black (8 total moves)
        # Black picks from available cells sequentially
        black_count = 0
        max_black = 0
        while black_count < 4 and not adapter.is_terminal(state):
            ...
            if not actions:
                break
            # Always take the first legal action
            state = adapter.apply_action(state, actions[0])
            if adapter.get_current_player(state) == "p_black" or adapter.is_terminal(state):
                black_count += 1
            # Track max black at any point
            black_pieces = [i for i, c in enumerate(state["_board"]) if c == "p_black"]
            max_black = max(max_black, len(black_pieces))

        # Black should have at most 3 pieces (FIFO evicted the oldest)
        black_pieces = [i for i, c in enumerate(state["_board"]) if c == "p_black"]
        assert len(black_pieces) <= 3
        # And at some point should have had 3 (before eviction) on a 3x3 board
        assert max_black >= 3
    ...
    def test_three_in_row_win(self, adapter: MoonChessAdapter):
        ...
        # We need to simulate careful placement without FIFO eviction
        # Black: (0,0), White: (1,0), Black: (0,1), White: (1,1), Black: (0,2)
        placements = [
            ("p_black", "cell_0_0"),
            ("p_white", "cell_1_0"),
            ("p_black", "cell_0_1"),
    ...
    def test_feature_vector(self, adapter: MoonChessAdapter):
        ...
        assert 0.0 <= vec.max() <= 3.0  # age encoding can be 0, 1, 2, 3
...
class TestMoonChessAdapterProtocol:
    ...
    def test_load_state_from_observation(self, adapter: MoonChessAdapter):
        """Simulate what VisionBridge does."""
        board_obs = [
            [None, "X", None],
            [None, None, None],
            ["O", None, None],
        ...
        assert state["_board"][1] == "p_black"
        assert state["_board"][6] == "p_white"
        assert adapter.get_current_player(state) == "p_black"
