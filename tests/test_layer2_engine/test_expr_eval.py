...
class TestCollectionPrimitives:
    ...
    def test_range_step_and_dynamic(self, ev):
        ctx = {'top': 7, 's': 2}
        assert _assert_consistent(
            ev, {'range': {'from': 1, 'to': {'var': '$top'}, 'step': {'var': '$s'}}}, ctx
        ) == [1, 3, 5]
    ...
    def test_group_by_field(self, ev):
        spec = {'group': {'list': [{'s': 'm', 'r': 1}, {'s': 'm', 'r': 2}, {'s': 'p', 'r': 1}],
                          'by': {'get': ['$node', 's']}}}
        groups = _assert_consistent(ev, spec)
...
class TestChoose:
    def test_existence(self, ev):
        spec = {'choose': {'items': [1, 2, 3, 4], 'k': 2,
                           'where': {'gt': [{'sum': {'var': '$c'}}, {'const': 6}]}}}
        assert _assert_consistent(ev, spec) is True  # 3+4 = 7

    def test_no_satisfying_combo(self, ev):
        spec = {'choose': {'items': [1, 2, 3], 'k': 2,
                           'where': {'gt': [{'sum': {'var': '$c'}}, {'const': 100}]}}}
    ...
    def test_dedupe_semantics(self, ev):
        ...
        spec = {'choose': {'items': items, 'k': 2, 'as': '$c',
                           'where': {'all': {'list': {'var': '$c'},
                                             'as': '$t',
                                             'where': {'eq': [{'var': '$t'}, {'const': 'm1'}]}}}}}
        # Only one m1 in the pool → no pair of m1s is possible.
        assert _assert_consistent(ev, spec) is False

    def test_dedupe_still_selects_different_kinds(self, ev):
        items = ['m1', 'm1', 'm1', 'm1', 'm2', 'm3']
        spec = {'choose': {'items': items, 'k': 2,
                           'where': {'contains': [{'var': '$c'}, {'const': 'm2'}]}}}
    ...
    def test_prefix_prunes_branch(self, ev):
        ...
        """
        spec = {
            'choose': {'items': ['a', 'b', 'c'], 'k': 2, 'as': '$c',
                       'prefix': {'eq': [{'at': [{'var': '$c'}, {'const': 0}]},
                                         {'const': 'a'}]},
    ...
    def test_prefix_sound_with_sum(self, ev):
        ...
        with_prefix = _assert_consistent(ev, {'choose': {
            **base, 'prefix': {'lte': [{'sum': {'var': '$c'}}, {'const': 6}]}}})
    ...
    def test_then_agg_max(self, ev):
        ...
        spec = {'choose': {'items': [1, 2, 3, 4], 'k': 3, 'as': '$c',
                           'then': {'sum': {'var': '$c'}}, 'agg': 'max'}}
        assert _assert_consistent(ev, spec) == 9  # 2+3+4

    def test_then_agg_min(self, ev):
        spec = {'choose': {'items': [1, 2, 3, 4], 'k': 2, 'as': '$c',
                           'then': {'sum': {'var': '$c'}}, 'agg': 'min'}}
    ...
    def test_choose_sorted_input(self, ev):
        ...
        a = {'choose': {'items': ['p2', 'm1', 'z5'], 'k': 1, 'as': '$c',
                        'where': {'contains': [{'var': '$c'}, {'const': 'm1'}]}}}
        b = {'choose': {'items': ['m1', 'p2', 'z5'], 'k': 1, 'as': '$c',
                        'where': {'contains': [{'var': '$c'}, {'const': 'm1'}]}}}
...
class TestAliasCall:
    ...
    def test_nested_calls_compile(self, ev):
        ...
        spec = {'call': ['sum3', {'call': ['double', {'const': 3}]},
                         {'const': 4}, {'const': 5}]}
    ...
    def test_indirect_recursion_rejected(self, ev):
        ev.set_functions({'two_step': _ALIASES['two_step'],
                          'two_step_b': _ALIASES['two_step_b']})
                    ...
                    'where': {
                        'all': {
                            ...
                            'where': {
                                'and': [
                                    {'and': [
                                        {'gte': [{'expr': 'c // size + (j - k) * d.dr'},
                                                 {'const': 0}]},
                                        {'lt': [{'expr': 'c // size + (j - k) * d.dr'},
                                                {'var': '$size'}]},
                                        {'gte': [{'expr': 'c % size + (j - k) * d.dc'},
                                                 {'const': 0}]},
                                        {'lt': [{'expr': 'c % size + (j - k) * d.dc'},
                                                {'var': '$size'}]},
                                    ]},
                                    {'eq': [
                                        {'at': [{'var': '$board'},
...
def _win_ctx(board, c, piece='p_black', size=3, win_length=3):
    return {'$board': board, 'piece': piece, 'c': c, 'size': size,
            'win_length': win_length,
            'dirs': [{'dr': 0, 'dc': 1}, {'dr': 1, 'dc': 0},
                     {'dr': 1, 'dc': 1}, {'dr': 1, 'dc': -1}]}
