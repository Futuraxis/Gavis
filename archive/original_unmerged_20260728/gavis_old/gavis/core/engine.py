...
Implements the SolverAdapter interface: all solvers interact with the game
...
class GameEngine:
    ...
    # ------------------------------------------------------------------
    # Public API (SolverAdapter)
    # ------------------------------------------------------------------

    def create_initial_state(self) -> dict:
        bs = self.rules.get('constants', {}).get('board_size', 9)
        return create_gomoku_state(bs)
    ...
    def apply_chance(self, state: dict, outcome: ChanceOutcome) -> dict:
        ...
        if last_cell_id and last_cell_id in new_state['nodes']:
    ...
    def get_observation(self, state: dict, player_id: str) -> dict:
        """Return the player's observation. For stochastic gomoku
        (perfect information), this is the full board state."""
        return {
    ...
    def get_info_set_key(self, state: dict, player_id: str) -> str:
        """Return a canonical info-set key for CFR. For perfect info
        games this is just the state hash."""
    ...
    def _expand_template(self, tmpl: dict, state: dict) -> list[ActionInstance]:
        ...
        for combo in combinations:
            ...
            # Keep full node data in params for expression evaluation
    ...
    def _eval_query_domain(self, query_expr: dict, state: dict, ctx: dict) -> list:
        ...
        if node_type == 'board_cell':
            # Fast path: use _board array to find empty cells directly
            board = state['_board']
            bs = state['board_size']
            nodes = state['nodes']
            results = []
            for idx in range(bs * bs):
                if board[idx] is None:
                    ...
                    # Use existing node or build on the fly
    ...
    def _build_context(self, state: dict, action: Optional[ActionInstance] = None) -> dict:
        ...
        if action is not None:
            ...
            ctx['actor'] = actor_node   # alias for $ resolution
            ctx['$action'] = action
            ctx['action'] = action
            ctx['$params'] = action.params
            ctx['params'] = action.params
            # Put full params in context for expression access
    ...
    # ------------------------------------------------------------------
    # State path access (set / get / inc ops)
    # ------------------------------------------------------------------
    ...
    def _get_state_path(self, state: dict, path: str) -> any:
        """Get value at dotted path, e.g. 'state.env.turn.round'."""
    ...
    def _set_state_path(self, state: dict, path: str, value: any):
        """Set value at dotted path, syncing _board for node props.

        Paths like 'state.nodes.cell_3_5.props.occupant' are intercepted
        to update both the node dict and the flat _board array.
        """
        ...
        if (len(parts) == 4 and parts[0] == 'nodes'
            ...
            cell_id = parts[1]
            # Parse coordinates from cell_id (e.g. "cell_3_5")
            try:
            ...
            # Sync nodes dict if it exists (may be lazy/empty in MCTS clones)
